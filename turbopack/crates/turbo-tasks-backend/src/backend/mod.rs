...
impl<B: BackingStorage> TurboTasksBackendInner<B> {
    ...
    fn try_read_task_cell(
        ...
        fn add_cell_dependency(
            ...
            if let Some(mut reader_task) = reader_task
                && (!task.immutable() || cfg!(feature = "verify_immutable"))
            {
                let _ = task.add_cell_dependents_value(cell, reader.unwrap());
                drop(task);
    ...
    fn snapshot_and_persist(
        ...
        enum PreprocessResult {
            None,
            MetaOnly(storage_schema::TaskStorage),
            DataOnly(storage_schema::TaskStorage),
            Both(storage_schema::TaskStorage),
        }
        let preprocess = |task_id: TaskId, inner: &storage_schema::TaskStorage| {
            ...
            match (meta_restored, data_restored) {
                (true, true) => {
                    ...
                    PreprocessResult::Both(inner.clone_snapshot())
                }
                (true, false) => {
                    // Only meta needs snapshot
                    PreprocessResult::MetaOnly(inner.clone_meta_snapshot())
                }
                (false, true) => {
                    // Only data needs snapshot
                    PreprocessResult::DataOnly(inner.clone_data_snapshot())
                }
        ...
        let process_snapshot = |task_id: TaskId, snapshot: Box<storage_schema::TaskStorage>| {
            if task_id.is_transient() {
        ...
        let execution_reason;
        {
            ...
            if self.should_track_dependencies() {
                ...
                let outdated_cell_dependencies_to_remove = task
                    ...
                    .filter(|&target| !task.has_cell_dependencies(&target))
                ...
                for target in outdated_cell_dependencies_to_remove {
                    task.remove_outdated_cell_dependencies(&target);
                }
    ...
    fn task_execution_completed_prepare(
        ...
        // Update or add cell type max indices
        ...
        if self.should_track_dependencies() {
            ...
            old_edges.extend(
                task.iter_cell_dependents_all()
                    .filter_map(|(&cell, &task)| {
                        if cell_counters
                            .get(&cell.type_id)
                            .is_none_or(|start_index| cell.index >= *start_index)
    ...
    fn read_task_collectibles(
        ...
        let mut collectibles = AutoMap::default();
        {
            ...
            if let Some(reader_id) = reader_id {
                let _ = task.add_collectibles_dependents_value(collectible_type, reader_id);
            }
        }
        if let Some(reader_id) = reader_id {
